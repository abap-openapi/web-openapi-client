...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique128 of abap.statements.loop(this.#mt_list)) {
      li_node.set(unique128);
...
class lcl_node {
  ...
  "SET_PARENT": {"visibility": "U", "parameters": {"II_PARENT": {"type": () => {return new abap.types.ABAPObject({qualifiedName: "IF_IXML_NODE", RTTIName: "\\INTERFACE=IF_IXML_NODE"});}, "is_optional": " "}}},
  "GET_PARENT": {"visibility": "U", "parameters": {"RI_PARENT": {"type": () => {return new abap.types.ABAPObject({qualifiedName: "IF_IXML_NODE", RTTIName: "\\INTERFACE=IF_IXML_NODE"});}, "is_optional": " "}}}};
  #mv_name;
  #mv_namespace;
  #mv_value;
  #mo_children;
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique129 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique129);
      ...
      let unique130 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique130++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique131 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique131++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique132 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique132++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.#mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique133 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique133++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async #parse_attributes(INPUT) {
    ...
    let unique136 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique136++);
