...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique125 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique125);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique126 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique126);
      ...
      let unique127 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique127++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique128 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique128++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique129 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique129++);
