...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique122 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique122);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique123 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique123);
