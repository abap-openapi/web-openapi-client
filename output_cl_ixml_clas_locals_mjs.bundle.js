...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique129 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique129);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique130 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique130);
...
class lcl_parser {
  ...
  async parse_attributes(INPUT) {
    ...
    let unique137 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique137++);
