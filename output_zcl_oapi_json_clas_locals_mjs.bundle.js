...
class lcl_stack {
  ...
  async get_full_name(INPUT) {
    ...
    "is_array": new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"}),
    "array_index": new abap.types.Integer({qualifiedName: "LCL_STACK=>TY_DATA-ARRAY_INDEX"})}, "lcl_stack=>ty_data", undefined, {}, {});
    for await (const unique75 of abap.statements.loop(this.#mt_data)) {
      ls_data.set(unique75);
...
class lcl_parser {
  ...
  async parse(INPUT) {
    ...
    for await (const unique77 of abap.statements.loop(lt_nodes)) {
      li_node.set(unique77);
      lv_index.set(abap.builtin.sy.get().tabix);
      let unique78 = li_node.get().if_sxml_node$type;
      if (abap.compare.eq(unique78, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique78, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
