...
class lcl_stack {
  ...
  async get_full_name() {
    ...
    let ls_data = new abap.types.Structure({"name": new abap.types.String({qualifiedName: "LCL_STACK=>TY_DATA-NAME"}), "is_array": new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"}), "array_index": new abap.types.Integer({qualifiedName: "LCL_STACK=>TY_DATA-ARRAY_INDEX"})}, "lcl_stack=>ty_data", undefined, {}, {});
    for await (const unique19 of abap.statements.loop(this.mt_data)) {
      ls_data.set(unique19);
...
class lcl_parser {
  ...
  async parse(INPUT) {
    ...
    let unique20 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique20++);
    ...
    for await (const unique21 of abap.statements.loop(lt_nodes)) {
      li_node.set(unique21);
      lv_index.set(abap.builtin.sy.get().tabix);
      let unique22 = li_node.get().if_sxml_node$type;
      if (abap.compare.eq(unique22, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique22, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
