...
class lcl_stack {
  ...
  static METHODS = {"PUSH": {"visibility": "U", "parameters": {"IV_NAME": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "I", "type_name": "StringType"}, "IV_TYPE": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "I", "type_name": "StringType"}}},
  "POP": {"visibility": "U", "parameters": {"RV_NAME": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "R", "type_name": "StringType"}}},
  "IS_ARRAY": {"visibility": "U", "parameters": {"RV_ARRAY": {"type": () => {return new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"});}, "is_optional": " ", "parm_kind": "R", "type_name": "CharacterType"}}},
  "GET_AND_INCREASE_INDEX": {"visibility": "U", "parameters": {"RV_INDEX": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "R", "type_name": "StringType"}}},
  "GET_FULL_NAME": {"visibility": "U", "parameters": {"RV_PATH": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "R", "type_name": "StringType"}}}};
  #mt_data;
  constructor() {
  ...
  async get_and_increase_index(INPUT) {
  ...
  async get_full_name(INPUT) {
    let rv_path = new abap.types.String({qualifiedName: "STRING"});
    let ls_data = new abap.types.Structure({
    "name": new abap.types.String({qualifiedName: "LCL_STACK=>TY_DATA-NAME"}),
    "is_array": new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"}),
    "array_index": new abap.types.Integer({qualifiedName: "LCL_STACK=>TY_DATA-ARRAY_INDEX"})}, "lcl_stack=>ty_data", undefined, {}, {});
    for await (const unique80 of abap.statements.loop(this.#mt_data)) {
      ls_data.set(unique80);
...
class lcl_parser {
  ...
  "value": new abap.types.String({qualifiedName: "TY_DATA-VALUE"})}, "ty_data", undefined, {}, {}), {"withHeader":false,"keyType":"DEFAULT","primaryKey":{"name":"primary_key","type":"STANDARD","isUnique":false,"keyFields":[]},"secondary":[]}, "ty_data_tt");}, "is_optional": " ", "parm_kind": "R", "type_name": "TableType"}, "IV_JSON": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " ", "parm_kind": "I", "type_name": "StringType"}}}};
  constructor() {
    this.me = new abap.types.ABAPObject();
    this.me.set(this);
    this.INTERNAL_ID = abap.internalIdCounter++;
  ...
  async parse(INPUT) {
    ...
    li_reader.set((await abap.Classes['CL_SXML_STRING_READER'].create({input: (await abap.Classes['CL_ABAP_CODEPAGE'].convert_to({source: iv_json, output: 1})), reader: 1})));
    ...
    for await (const unique82 of abap.statements.loop(lt_nodes)) {
      li_node.set(unique82);
      lv_index.set(abap.builtin.sy.get().tabix);
      let unique83 = li_node.get().if_sxml_node$type;
      if (abap.compare.eq(unique83, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
        ...
        abap.statements.readTable(lt_attributes,{index: abap.IntegerFactory.get(1),
          into: li_attribute});
        if (abap.compare.eq(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
          lv_push.set((await li_attribute.get().if_sxml_attribute$get_value({value: 1})));
        } else if (abap.compare.eq((await lo_stack.get().is_array({rv_array: 1})), abap.builtin.abap_true)) {
          lv_push.set((await lo_stack.get().get_and_increase_index({rv_index: 1})));
        }
        if (abap.compare.initial(lv_push) === false) {
          ls_data.clear();
          ls_data.get().parent.set((await lo_stack.get().get_full_name({rv_path: 1})));
          ...
          if (abap.compare.eq(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0)) && abap.compare.eq(li_next.get().if_sxml_node$type, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
            ...
            ls_data.get().value.set((await li_value.get().if_sxml_value_node$get_value({value: 1})));
          }
      ...
      } else if (abap.compare.eq(unique83, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        await abap.statements.cast(li_close, li_node);
        lv_name.set((await lo_stack.get().pop({rv_name: 1})));
