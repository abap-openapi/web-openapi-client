...
class lcl_input_arguments {
  ...
  async if_ftd_input_arguments$get_importing_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, new abap.types.Integer().set(0))) {
      const unique217 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique217.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 18};
      throw unique217;
    }
...
class lcl_invoker {
  ...
  static async invoke(INPUT) {
    ...
    for await (const unique218 of abap.statements.loop(lo_result.get().mt_exporting)) {
      ls_exporting.set(unique218);
