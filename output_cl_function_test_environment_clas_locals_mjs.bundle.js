...
class lcl_input_arguments {
  ...
  async if_ftd_input_arguments$get_importing_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique149 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique149.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 20};
      throw unique149;
    }
  ...
  async if_ftd_input_arguments$get_table_parameter(INPUT) {
    ...
    abap.statements.readTable(this.mt_tables,{into: ls_row,
      ...
      withKeySimple: {"name": name}});
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique150 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique150.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 29};
      throw unique150;
    }
    result.set(ls_row.get().value);
    return result;
  ...
  async if_ftd_input_arguments$get_changing_parameter(INPUT) {
    ...
    abap.statements.readTable(this.mt_changing,{into: ls_row,
      ...
      withKeySimple: {"name": name}});
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique151 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique151.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 38};
      throw unique151;
    }
    result.set(ls_row.get().value);
    return result;
...
class lcl_invocation_result {
  ...
  async if_ftd_invocation_result$get_output_configuration(INPUT) {
  ...
  async if_ftd_output_configuration$set_exporting_parameter(INPUT) {
    ...
    abap.statements.getReference(ls_row.get().value, value);
...
class lcl_invoker {
  ...
  static ATTRIBUTES = {};
  static METHODS = {"INVOKE": {"visibility": "U", "parameters": {"FMINPUT": {"type": () => {return new abap.types.Character(4);}, "is_optional": " ", "parm_kind": "I", "type_name": "AnyType"}, "ANSWER": {"type": () => {return new abap.types.ABAPObject({qualifiedName: "IF_FTD_INVOCATION_ANSWER", RTTIName: "\\INTERFACE=IF_FTD_INVOCATION_ANSWER"});}, "is_optional": " ", "parm_kind": "I", "type_name": "ObjectReferenceType"}}}};
  constructor() {
    this.me = new abap.types.ABAPObject();
    this.me.set(this);
    this.INTERNAL_ID = abap.internalIdCounter++;
  ...
  static async invoke(INPUT) {
    ...
    for await (const unique152 of abap.statements.loop(lo_result.get().mt_exporting)) {
      ls_exporting.set(unique152);
      fminput.importing[ls_exporting.get().name.get().toLowerCase().trimEnd()].set(ls_exporting.get().value.dereference());
    }
    for await (const unique153 of abap.statements.loop(lo_result.get().mt_tables)) {
      ls_table.set(unique153);
...
class lcl_double {
  ...
  static METHODS = {"CONSTRUCTOR": {"visibility": "U", "parameters": {"IV_NAME": {"type": () => {return new abap.types.Character(30, {"qualifiedName":"SXCO_FM_NAME","ddicName":"SXCO_FM_NAME","description":"Function module name"});}, "is_optional": " ", "parm_kind": "I", "type_name": "CharacterType"}}}};
  #mv_name;
  constructor() {
    this.me = new abap.types.ABAPObject();
    this.me.set(this);
  ...
  async if_function_testdouble$configure_call(INPUT) {
  ...
  async if_ftd_input_config_setter$ignore_all_parameters(INPUT) {
