...
class lcl_input_arguments {
  ...
  async if_ftd_input_arguments$get_importing_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique249 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique249.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 19};
      throw unique249;
    }
  ...
  async if_ftd_input_arguments$get_table_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      ...
      unique250.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 28};
      throw unique250;
    }
    result.set(ls_row.get().value);
    return result;
...
class lcl_invoker {
  ...
  static async invoke(INPUT) {
    ...
    for await (const unique252 of abap.statements.loop(lo_result.get().mt_tables)) {
      ls_table.set(unique252);
