...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    if (abap.compare.eq(unique157, new abap.types.Character(1).set('u')) || abap.compare.eq(unique157, new abap.types.Character(1).set('v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique157, new abap.types.Character(1).set('h'))) {
  ...
  static async dump_structure(INPUT) {
    ...
    for await (const unique158 of abap.statements.loop(lt_components)) {
      lv_name.set(unique158);
