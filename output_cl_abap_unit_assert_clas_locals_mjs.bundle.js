...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    if (abap.compare.eq(unique149, new abap.types.Character({length: 1}).set('u')) || abap.compare.eq(unique149, new abap.types.Character({length: 1}).set('v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique149, new abap.types.Character({length: 1}).set('h'))) {
