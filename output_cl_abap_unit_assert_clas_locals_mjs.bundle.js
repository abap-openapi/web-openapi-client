...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    let unique207 = lv_type;
    if (abap.compare.eq(unique207, new abap.types.Character(1).set('u')) || abap.compare.eq(unique207, new abap.types.Character(1).set('v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique207, new abap.types.Character(1).set('h'))) {
      rv_str.set(new abap.types.String().set(`[itab]`));
    } else if (abap.compare.eq(unique207, new abap.types.Character(1).set('r'))) {
  ...
  static async dump_structure(INPUT) {
    ...
    for await (const unique208 of abap.statements.loop(lt_components)) {
      lv_name.set(unique208);
