...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    let unique235 = lv_type;
    if (abap.compare.eq(unique235, abap.CharacterFactory.get(1, 'u')) || abap.compare.eq(unique235, abap.CharacterFactory.get(1, 'v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique235, abap.CharacterFactory.get(1, 'h'))) {
      rv_str.set(new abap.types.String().set(`[itab]`));
    } else if (abap.compare.eq(unique235, abap.CharacterFactory.get(1, 'r'))) {
  ...
  static async dump_structure(INPUT) {
    ...
    for await (const unique236 of abap.statements.loop(lt_components)) {
      lv_name.set(unique236);
