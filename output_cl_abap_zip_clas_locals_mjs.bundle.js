...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique76 = new abap.types.Integer().set(256).get();
      for (let unique77 = 0; unique77 < unique76; unique77++) {
        ...
        const unique78 = abap.IntegerFactory.get(8).get();
        for (let unique79 = 0; unique79 < unique78; unique79++) {
          abap.builtin.sy.get().index.set(unique79 + 1);
    ...
    const unique80 = len.get();
    for (let unique81 = 0; unique81 < unique80; unique81++) {
      abap.builtin.sy.get().index.set(unique81 + 1);
