...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique84 = new abap.types.Integer().set(256).get();
      for (let unique85 = 0; unique85 < unique84; unique85++) {
        abap.builtin.sy.get().index.set(unique85 + 1);
        ...
        const unique86 = abap.IntegerFactory.get(8).get();
        for (let unique87 = 0; unique87 < unique86; unique87++) {
          abap.builtin.sy.get().index.set(unique87 + 1);
