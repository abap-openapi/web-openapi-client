...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    } else if (abap.compare.eq(unique147, new abap.types.Character(6).set('string')) || abap.compare.eq(unique147, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique147, new abap.types.Character(6).set('number')) || abap.compare.eq(unique147, new abap.types.Character(4).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    if (abap.compare.eq(unique148, new abap.types.Character(6).set('string'))) {
      lv_type.set(new abap.types.Character(3).set('str'));
    } else if (abap.compare.eq(unique148, new abap.types.Character(6).set('number'))) {
      lv_type.set(new abap.types.Character(3).set('num'));
    } else if (abap.compare.eq(unique148, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique149 = lv_length.get();
    for (let unique150 = 0; unique150 < unique149; unique150++) {
      abap.builtin.sy.get().index.set(unique150 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique151 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique151);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique152 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique152);
      let unique153 = ls_parsed.get().type;
      if (abap.compare.eq(unique153, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique153, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        li_node.set(await (new abap.Classes['CLAS-CL_SXML_STRING_READER-LCL_CLOSE_NODE']()).constructor_({name: ls_parsed.get().name}));
      } else if (abap.compare.eq(unique153, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique154 = this.if_sxml_reader$node_type;
