...
class lcl_json_parser {
  ...
  async parse(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      ...
      const unique193 = await (new abap.Classes['CX_SXML_PARSE_ERROR']()).constructor_({xml_offset: lv_xml_offset});
      unique193.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sxml_string_reader.clas.locals_imp.abap","INTERNAL_LINE": 64};
      throw unique193;
    }
  ...
  async traverse(INPUT) {
    ...
    let unique194 = lv_type;
    if (abap.compare.eq(unique194, new abap.types.Character(6).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique194, new abap.types.Character(5).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique194, new abap.types.Character(6).set('string')) || abap.compare.eq(unique194, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique194, new abap.types.Character(6).set('number')) || abap.compare.eq(unique194, new abap.types.Character(4).set('null'))) {
      iv_json = iv_json.value + "";
      let unique195 = lv_type;
      if (abap.compare.eq(unique195, new abap.types.Character(6).set('string'))) {
        lv_type.set(new abap.types.Character(3).set('str'));
      } else if (abap.compare.eq(unique195, new abap.types.Character(6).set('number'))) {
        lv_type.set(new abap.types.Character(3).set('num'));
      } else if (abap.compare.eq(unique195, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique196 = lv_length.get();
    for (let unique197 = 0; unique197 < unique196; unique197++) {
      abap.builtin.sy.get().index.set(unique197 + 1);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique198 of abap.statements.loop(lt_parsed.dereference())) {
      fs_ls_parsed_.assign(unique198);
      let unique199 = fs_ls_parsed_.get().type;
      ...
      } else if (abap.compare.eq(unique199, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$read_next_node() {
    ...
    if (abap.compare.initial(node) === false) {
      ...
      let unique200 = this.if_sxml_reader$node_type;
