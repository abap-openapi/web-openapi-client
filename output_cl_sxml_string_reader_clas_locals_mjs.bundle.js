...
class lcl_json_parser {
  ...
  async parse(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      ...
      unique191.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sxml_string_reader.clas.locals_imp.abap","INTERNAL_LINE": 64};
      throw unique191;
    }
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique196 of abap.statements.loop(lt_parsed.dereference())) {
      fs_ls_parsed_.assign(unique196);
      let unique197 = fs_ls_parsed_.get().type;
  ...
  async if_sxml_reader$read_next_node() {
    ...
    if (abap.compare.initial(node) === false) {
      ...
      let unique198 = this.if_sxml_reader$node_type;
