...
class lcl_json_parser {
  ...
  async parse(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      ...
      const unique196 = await (new abap.Classes['CX_SXML_PARSE_ERROR']()).constructor_({xml_offset: lv_xml_offset});
      unique196.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sxml_string_reader.clas.locals_imp.abap","INTERNAL_LINE": 64};
      throw unique196;
    }
  ...
  async traverse(INPUT) {
    ...
    let unique197 = lv_type;
    if (abap.compare.eq(unique197, new abap.types.Character(6).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique197, new abap.types.Character(5).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique197, new abap.types.Character(6).set('string')) || abap.compare.eq(unique197, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique197, new abap.types.Character(6).set('number')) || abap.compare.eq(unique197, new abap.types.Character(4).set('null'))) {
      iv_json = iv_json.value + "";
      let unique198 = lv_type;
      if (abap.compare.eq(unique198, new abap.types.Character(6).set('string'))) {
        lv_type.set(new abap.types.Character(3).set('str'));
      } else if (abap.compare.eq(unique198, new abap.types.Character(6).set('number'))) {
        lv_type.set(new abap.types.Character(3).set('num'));
      } else if (abap.compare.eq(unique198, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique199 = lv_length.get();
    for (let unique200 = 0; unique200 < unique199; unique200++) {
      abap.builtin.sy.get().index.set(unique200 + 1);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique201 of abap.statements.loop(lt_parsed.dereference())) {
      fs_ls_parsed_.assign(unique201);
      let unique202 = fs_ls_parsed_.get().type;
      if (abap.compare.eq(unique202, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique202, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        li_node.set(await (new abap.Classes['CLAS-CL_SXML_STRING_READER-LCL_CLOSE_NODE']()).constructor_({name: fs_ls_parsed_.get().name}));
      } else if (abap.compare.eq(unique202, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$read_next_node() {
    ...
    if (abap.compare.initial(node) === false) {
      ...
      let unique203 = this.if_sxml_reader$node_type;
      if (abap.compare.eq(unique203, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique203, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
      ...
      } else if (abap.compare.eq(unique203, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
