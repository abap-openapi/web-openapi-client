...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    let unique151 = lv_type;
    if (abap.compare.eq(unique151, new abap.types.Character(6).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique151, new abap.types.Character(5).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique151, new abap.types.Character(6).set('string')) || abap.compare.eq(unique151, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique151, new abap.types.Character(6).set('number')) || abap.compare.eq(unique151, new abap.types.Character(4).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    let unique152 = lv_type;
    if (abap.compare.eq(unique152, new abap.types.Character(6).set('string'))) {
      lv_type.set(new abap.types.Character(3).set('str'));
    } else if (abap.compare.eq(unique152, new abap.types.Character(6).set('number'))) {
      lv_type.set(new abap.types.Character(3).set('num'));
    } else if (abap.compare.eq(unique152, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique153 = lv_length.get();
    for (let unique154 = 0; unique154 < unique153; unique154++) {
      abap.builtin.sy.get().index.set(unique154 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique155 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique155);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique156 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique156);
      let unique157 = ls_parsed.get().type;
      if (abap.compare.eq(unique157, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique157, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        li_node.set(await (new abap.Classes['CLAS-CL_SXML_STRING_READER-LCL_CLOSE_NODE']()).constructor_({name: ls_parsed.get().name}));
      } else if (abap.compare.eq(unique157, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique158 = this.if_sxml_reader$node_type;
