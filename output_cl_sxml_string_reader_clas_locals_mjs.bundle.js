...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    } else if (abap.compare.eq(unique146, new abap.types.Character(6).set('string')) || abap.compare.eq(unique146, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique146, new abap.types.Character(6).set('number')) || abap.compare.eq(unique146, new abap.types.Character(4).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    if (abap.compare.eq(unique147, new abap.types.Character(6).set('string'))) {
      lv_type.set(new abap.types.Character(3).set('str'));
    } else if (abap.compare.eq(unique147, new abap.types.Character(6).set('number'))) {
      lv_type.set(new abap.types.Character(3).set('num'));
    } else if (abap.compare.eq(unique147, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique148 = lv_length.get();
    for (let unique149 = 0; unique149 < unique148; unique149++) {
      abap.builtin.sy.get().index.set(unique149 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique150 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique150);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique151 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique151);
      let unique152 = ls_parsed.get().type;
      ...
      } else if (abap.compare.eq(unique152, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique153 = this.if_sxml_reader$node_type;
