...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique152, new abap.types.Character(6).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique152, new abap.types.Character(5).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique152, new abap.types.Character(6).set('string')) || abap.compare.eq(unique152, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique152, new abap.types.Character(6).set('number')) || abap.compare.eq(unique152, new abap.types.Character(4).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    if (abap.compare.eq(unique153, new abap.types.Character(6).set('string'))) {
      lv_type.set(new abap.types.Character(3).set('str'));
    } else if (abap.compare.eq(unique153, new abap.types.Character(6).set('number'))) {
      lv_type.set(new abap.types.Character(3).set('num'));
    } else if (abap.compare.eq(unique153, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique154 = lv_length.get();
    for (let unique155 = 0; unique155 < unique154; unique155++) {
      abap.builtin.sy.get().index.set(unique155 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique156 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique156);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique157 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique157);
      let unique158 = ls_parsed.get().type;
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique159 = this.if_sxml_reader$node_type;
