...
class lcl_json_parser {
  ...
  "PARSE": {"visibility": "U", "parameters": {"RT_NODES": {"type": () => {return abap.types.TableFactory.construct(new abap.types.Structure({"type": new abap.types.Integer({qualifiedName: "IF_SXML_NODE=>NODE_TYPE"}), "name": new abap.types.String({qualifiedName: "LCL_JSON_PARSER=>TY_NODE-NAME"}), "key": new abap.types.String({qualifiedName: "LCL_JSON_PARSER=>TY_NODE-KEY"}), "value": new abap.types.String({qualifiedName: "LCL_JSON_PARSER=>TY_NODE-VALUE"})}, "lcl_json_parser=>ty_node"), {"withHeader":false,"keyType":"DEFAULT","primaryKey":{"name":"primary_key","type":"STANDARD","isUnique":false,"keyFields":[]},"secondary":[]}, "lcl_json_parser=>ty_nodes");}, "is_optional": " "}, "IV_JSON": {"type": () => {return new abap.types.String({qualifiedName: "STRING"});}, "is_optional": " "}}}};
  constructor() {
  ...
  async parse(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      ...
      const unique180 = await (new abap.Classes['CX_SXML_PARSE_ERROR']()).constructor_({xml_offset: lv_xml_offset});
      unique180.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sxml_string_reader.clas.locals_imp.abap","INTERNAL_LINE": 68};
      throw unique180;
    }
  ...
  async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique181, new abap.types.Character(6).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique181, new abap.types.Character(5).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique181, new abap.types.Character(6).set('string')) || abap.compare.eq(unique181, new abap.types.Character(7).set('boolean')) || abap.compare.eq(unique181, new abap.types.Character(6).set('number')) || abap.compare.eq(unique181, new abap.types.Character(4).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    let unique182 = lv_type;
    if (abap.compare.eq(unique182, new abap.types.Character(6).set('string'))) {
      lv_type.set(new abap.types.Character(3).set('str'));
    } else if (abap.compare.eq(unique182, new abap.types.Character(6).set('number'))) {
      lv_type.set(new abap.types.Character(3).set('num'));
    } else if (abap.compare.eq(unique182, new abap.types.Character(7).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique183 = lv_length.get();
    for (let unique184 = 0; unique184 < unique183; unique184++) {
      abap.builtin.sy.get().index.set(unique184 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique185 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique185);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique186 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique186);
      let unique187 = ls_parsed.get().type;
      if (abap.compare.eq(unique187, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      ...
      } else if (abap.compare.eq(unique187, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        li_node.set(await (new abap.Classes['CLAS-CL_SXML_STRING_READER-LCL_CLOSE_NODE']()).constructor_({name: ls_parsed.get().name}));
      } else if (abap.compare.eq(unique187, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique188 = this.if_sxml_reader$node_type;
    if (abap.compare.eq(unique188, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
      await abap.statements.cast(open, node);
      this.if_sxml_reader$name.set(open.get().if_sxml_open_element$qname.get().name);
    } else if (abap.compare.eq(unique188, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
