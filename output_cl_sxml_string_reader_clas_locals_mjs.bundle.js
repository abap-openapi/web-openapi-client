...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique138, new abap.types.Character({length: 6}).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique138, new abap.types.Character({length: 5}).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique138, new abap.types.Character({length: 6}).set('string')) || abap.compare.eq(unique138, new abap.types.Character({length: 7}).set('boolean')) || abap.compare.eq(unique138, new abap.types.Character({length: 6}).set('number')) || abap.compare.eq(unique138, new abap.types.Character({length: 4}).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    if (abap.compare.eq(unique139, new abap.types.Character({length: 6}).set('string'))) {
      lv_type.set(new abap.types.Character({length: 3}).set('str'));
    } else if (abap.compare.eq(unique139, new abap.types.Character({length: 6}).set('number'))) {
      lv_type.set(new abap.types.Character({length: 3}).set('num'));
    } else if (abap.compare.eq(unique139, new abap.types.Character({length: 7}).set('boolean'))) {
  ...
  async traverse_array(INPUT) {
    ...
    const unique140 = lv_length.get();
    for (let unique141 = 0; unique141 < unique140; unique141++) {
      abap.builtin.sy.get().index.set(unique141 + 1);
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique142 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique142);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique143 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique143);
      let unique144 = ls_parsed.get().type;
