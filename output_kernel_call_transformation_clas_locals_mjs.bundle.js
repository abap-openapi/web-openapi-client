...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique155 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique155);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      if (abap.compare.eq(unique156, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique156, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique159 = lo_type.get().type_kind;
        if (abap.compare.eq(unique159, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
          ...
          if (INPUT.source.constructor.name === "Object") {
              ...
              class lcl_object_to_ixml {
                ...
                static async traverse(INPUT) {
                  ...
                  let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                  lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
                  let unique165 = lo_type.get().kind;
                  if (abap.compare.eq(unique165, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                    await abap.statements.cast(lo_struc, lo_type);
                    lt_comps.set((await lo_struc.get().get_components()));
                    abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
                    for await (const unique166 of abap.statements.loop(lt_comps)) {
                      ls_compo.set(unique166);
                  ...
                  } else if (abap.compare.eq(unique165, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                    await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting(iv_ref.dereference())}`)});
                  } else if (abap.compare.eq(unique165, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                    abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
