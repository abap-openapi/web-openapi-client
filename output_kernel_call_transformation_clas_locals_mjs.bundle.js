...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique149 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique149);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique150 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique150);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      if (abap.compare.eq(unique151, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique151, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique154 = lo_type.get().type_kind;
        if (abap.compare.eq(unique154, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
    ...
    static async run(INPUT) {
      ...
      for (const name in INPUT.source) {
        ...
        static async traverse_write_type(INPUT) {
          ...
          let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
          let unique155 = lo_type.get().type_kind;
          if (abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique155, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
        ...
        static async traverse_write(INPUT) {
          ...
          let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
          let unique156 = lo_type.get().kind;
          if (abap.compare.eq(unique156, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(6).set('object')});
            await abap.statements.cast(lo_struc, lo_type);
            lt_comps.set((await lo_struc.get().get_components()));
            abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
            for await (const unique157 of abap.statements.loop(lt_comps)) {
              ls_compo.set(unique157);
              abap.statements.assign({component: ls_compo.get().name, target: fs_field_, source: fs_any_});
              lv_ref.assign(fs_field_.getPointer());
            ...
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$close_element();
          } else if (abap.compare.eq(unique156, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$write_value({value: iv_ref.dereference()});
          } else if (abap.compare.eq(unique156, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(5).set('array')});
            abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
            for await (const unique158 of abap.statements.loop(fs_table_)) {
              fs_any_.assign(unique158);
              lv_ref.assign(fs_any_.getPointer());
          ...
          if (INPUT.source.constructor.name === "Object") {
              ...
              class lcl_object_to_ixml {
                ...
                static async run(INPUT) {
                  ...
                  for await (const unique159 of abap.statements.loop(lt_stab)) {
                    ls_stab.set(unique159);
