...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique147 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique147);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique148 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique148);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      if (abap.compare.eq(unique149, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique150 of abap.statements.loop(lt_comps)) {
          ls_compo.set(unique150);
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique151 of abap.statements.loop(fs_table_)) {
          fs_any_.assign(unique151);
          lv_ref.assign(fs_any_.getPointer());
        ...
        if (abap.compare.eq(unique152, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
    ...
    static async run(INPUT) {
      ...
      for (const name in INPUT.source) {
        ...
        static async traverse_write_type(INPUT) {
          ...
          let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
          let unique153 = lo_type.get().type_kind;
          if (abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
          ...
          if (INPUT.source.constructor.name === "Object") {
              ...
              class lcl_object_to_ixml {
                ...
                static async run(INPUT) {
                  ...
                  for await (const unique157 of abap.statements.loop(lt_stab)) {
                    ls_stab.set(unique157);
                ...
                static async traverse(INPUT) {
                  ...
                  let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                  lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
                  let unique158 = lo_type.get().kind;
                  if (abap.compare.eq(unique158, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                    await abap.statements.cast(lo_struc, lo_type);
                    lt_comps.set((await lo_struc.get().get_components()));
                    abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
                    for await (const unique159 of abap.statements.loop(lt_comps)) {
                      ls_compo.set(unique159);
                  ...
                  } else if (abap.compare.eq(unique158, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                    await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting(iv_ref.dereference())}`)});
                  } else if (abap.compare.eq(unique158, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                    abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
                    for await (const unique160 of abap.statements.loop(fs_table_)) {
                      fs_any_.assign(unique160);
