...
class lcl_heap {
  ...
  async add(INPUT) {
    ...
    for await (const unique135 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique135);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique136 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique136);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      let unique137 = lo_type.get().kind;
      if (abap.compare.eq(unique137, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique138 of abap.statements.loop(lt_comps)) {
          ls_compo.set(unique138);
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique137, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      ...
      } else if (abap.compare.eq(unique137, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique139 of abap.statements.loop(fs_table_)) {
          fs_any_.assign(unique139);
          lv_ref.assign(fs_any_.getPointer());
          rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
        }
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique137, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique140 = lo_type.get().type_kind;
        if (abap.compare.eq(unique140, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
