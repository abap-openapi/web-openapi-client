...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique140 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique140);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique141 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique141);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      let unique142 = lo_type.get().kind;
      if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique143 of abap.statements.loop(lt_comps)) {
          ls_compo.set(unique143);
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      ...
      } else if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique144 of abap.statements.loop(fs_table_)) {
          fs_any_.assign(unique144);
          lv_ref.assign(fs_any_.getPointer());
          rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
        }
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique145 = lo_type.get().type_kind;
        if (abap.compare.eq(unique145, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
    ...
    static async run(INPUT) {
      ...
      for (const name in INPUT.source) {
        ...
        static async traverse_write_type(INPUT) {
          ...
          let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
          let unique146 = lo_type.get().type_kind;
          if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
        ...
        static async traverse_write(INPUT) {
          ...
          let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
          lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
          let unique147 = lo_type.get().kind;
          if (abap.compare.eq(unique147, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(6).set('object')});
            await abap.statements.cast(lo_struc, lo_type);
            lt_comps.set((await lo_struc.get().get_components()));
            abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
            for await (const unique148 of abap.statements.loop(lt_comps)) {
              ls_compo.set(unique148);
              abap.statements.assign({component: ls_compo.get().name, target: fs_field_, source: fs_any_});
              lv_ref.assign(fs_field_.getPointer());
            ...
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$close_element();
          } else if (abap.compare.eq(unique147, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$write_value({value: (iv_ref).dereference()});
          } else if (abap.compare.eq(unique147, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
            await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(5).set('array')});
            abap.statements.assign({target: fs_table_, source: (iv_ref).dereference()});
            for await (const unique149 of abap.statements.loop(fs_table_)) {
              fs_any_.assign(unique149);
              lv_ref.assign(fs_any_.getPointer());
          ...
          if (INPUT.source.constructor.name === "Object") {
              ...
              class lcl_object_to_ixml {
                ...
                static async run(INPUT) {
                  ...
                  for await (const unique150 of abap.statements.loop(lt_stab)) {
                    ls_stab.set(unique150);
                ...
                static async traverse(INPUT) {
                  ...
                  let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                  lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
                  let unique151 = lo_type.get().kind;
                  if (abap.compare.eq(unique151, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                    await abap.statements.cast(lo_struc, lo_type);
                    lt_comps.set((await lo_struc.get().get_components()));
                    abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
                    for await (const unique152 of abap.statements.loop(lt_comps)) {
                      ls_compo.set(unique152);
                  ...
                  } else if (abap.compare.eq(unique151, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                    await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting((iv_ref).dereference())}`)});
                  } else if (abap.compare.eq(unique151, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                    abap.statements.assign({target: fs_table_, source: (iv_ref).dereference()});
                    for await (const unique153 of abap.statements.loop(fs_table_)) {
                      fs_any_.assign(unique153);
