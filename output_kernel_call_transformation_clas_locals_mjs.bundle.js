...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique164 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique164);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique165 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique165);
...
class lcl_data_to_xml {
  ...
  async run(INPUT) {
    ...
    let unique166 = lo_type.get().kind;
    if (abap.compare.eq(unique166, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique167 of abap.statements.loop(lt_comps)) {
        ls_compo.set(unique167);
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique166, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique166, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique168 of abap.statements.loop(fs_table_)) {
        fs_any_.assign(unique168);
        abap.statements.getReference(lv_ref, fs_any_);
        rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
      }
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique166, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      let unique169 = lo_type.get().type_kind;
      if (abap.compare.eq(unique169, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
  ...
  static async run(INPUT) {
    ...
    for (const name in INPUT.source) {
      ...
      static async traverse_write_type(INPUT) {
        ...
        let unique170 = lo_type.get().type_kind;
        if (abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique170, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
      ...
      static async traverse_write(INPUT) {
        ...
        abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
        lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: fs_any_})));
        let unique171 = lo_type.get().kind;
        if (abap.compare.eq(unique171, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: abap.CharacterFactory.get(6, 'object')});
          await abap.statements.cast(lo_struc, lo_type);
          lt_comps.set((await lo_struc.get().get_components()));
          for await (const unique172 of abap.statements.loop(lt_comps)) {
            ls_compo.set(unique172);
            abap.statements.assign({component: ls_compo.get().name, target: fs_field_, source: fs_any_});
            abap.statements.getReference(lv_ref, fs_field_);
          ...
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$close_element();
        } else if (abap.compare.eq(unique171, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$write_value({value: fs_any_});
        } else if (abap.compare.eq(unique171, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: abap.CharacterFactory.get(5, 'array')});
          abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
          for await (const unique173 of abap.statements.loop(fs_table_)) {
            fs_any_.assign(unique173);
            abap.statements.getReference(lv_ref, fs_any_);
        ...
        if (INPUT.source.constructor.name === "Object") {
            ...
            class lcl_object_to_ixml {
              ...
              static async traverse(INPUT) {
                ...
                abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
                lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: fs_any_})));
                let unique175 = lo_type.get().kind;
                if (abap.compare.eq(unique175, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                  await abap.statements.cast(lo_struc, lo_type);
                  lt_comps.set((await lo_struc.get().get_components()));
                ...
                } else if (abap.compare.eq(unique175, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                  await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting(fs_any_)}`)});
                } else if (abap.compare.eq(unique175, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                  abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
                  for await (const unique177 of abap.statements.loop(fs_table_)) {
                    fs_any_.assign(unique177);
