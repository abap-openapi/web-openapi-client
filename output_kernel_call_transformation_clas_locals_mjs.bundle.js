...
class lcl_heap {
  ...
  async add(INPUT) {
    ...
    for await (const unique126 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique126);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique127 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique127);
  ...
  class lcl_data_to_xml {
    ...
    static METHODS = {"CONSTRUCTOR": {"visibility": "U", "parameters": {"IO_HEAP": {"type": () => {return new abap.types.ABAPObject({qualifiedName: "LCL_HEAP", RTTIName: "\\CLASS-POOL=KERNEL_CALL_TRANSFORMATION\\CLASS=LCL_HEAP"});}, "is_optional": " "}}},
    ...
    async run(INPUT) {
      let rv_xml = new abap.types.String({qualifiedName: "STRING"});
      let iv_name = new abap.types.Character();
      if (INPUT && INPUT.iv_name) {iv_name = INPUT.iv_name;}
      ...
      let unique128 = lo_type.get().kind;
      if (abap.compare.eq(unique128, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        for await (const unique129 of abap.statements.loop(lt_comps)) {
          ls_compo.set(unique129);
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique128, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      ...
      } else if (abap.compare.eq(unique128, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
        abap.statements.assign({target: fs_table_, source: (iv_ref).dereference()});
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
        for await (const unique130 of abap.statements.loop(fs_table_)) {
          fs_any_.assign(unique130);
          lv_ref.assign(fs_any_.getPointer());
          rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
        }
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique128, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique131 = lo_type.get().type_kind;
        if (abap.compare.eq(unique131, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
